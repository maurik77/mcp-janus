...
> Requires PowerShell 5.1+ (built into Windows 10/11). Run from the repository root.
...
#   → writes .env.keycloak-dev.ps1 with the client secret

# 3. Copy Keycloak config over the default
Copy-Item config.keycloak-dev.yaml config.yaml

# 4. Build the proxy
task build

# 5. Load env vars and start the proxy
. .\.env.keycloak-dev.ps1
$env:CONFIG_PATH = "."; .\bin\mcpproxy.exe
...
# Linux / macOS
.env.keycloak-dev                 ← Generated by setup-keycloak.sh (gitignored)
scripts/keycloak/
...
# Windows / PowerShell
.env.keycloak-dev.ps1             ← Generated by setup-keycloak.ps1 (gitignored)
scripts/keycloak/
  setup-keycloak.ps1              ← One-shot Keycloak bootstrap
  verify-idp.ps1                  ← Sanity check (no browser needed)
  test-proxy-flow.ps1             ← Full E2E test with browser + PKCE
```

The generated `.env.keycloak-dev` / `.env.keycloak-dev.ps1` files contain the Keycloak client secret — both are in `.gitignore` and must not be committed.

### PowerShell execution policy

On Windows, PowerShell scripts may be blocked by the default execution policy. To allow local scripts:
```powershell
Set-ExecutionPolicy -Scope CurrentUser RemoteSigned
```
This only affects the current user and does not lower system-wide security.
