...
func TestFetchJWKS(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		jwks, err := fetchJWKS(ts.URL, false)
	...
	t.Run("network error", func(t *testing.T) {
		_, err := fetchJWKS("http://127.0.0.1:1/nonexistent", false)
		assert.Error(t, err)
...
func TestFetchOpenIDConfiguration(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		config, err := fetchOpenIDConfiguration(ts.URL, false)
		require.NoError(t, err)
	...
	t.Run("http error status", func(t *testing.T) {
		...
		_, err := fetchOpenIDConfiguration(ts.URL, false)
		assert.Error(t, err)
	...
	t.Run("network error", func(t *testing.T) {
		_, err := fetchOpenIDConfiguration("http://127.0.0.1:1/nonexistent", false)
		assert.Error(t, err)
...
func TestJWK_RSAKey(t *testing.T) {
	t.Run("valid RSA key round-trip", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, 0, origKey.N.Cmp(pubKey.N), "modulus mismatch")
		assert.Equal(t, origKey.E, pubKey.E, "exponent mismatch")
	})
	...
	t.Run("invalid E base64", func(t *testing.T) {
		...
		jwk := JWK{
			Kty: "RSA",
			N:   base64.RawURLEncoding.EncodeToString(origKey.N.Bytes()),
			E:   "!!!invalid!!!",
		}
		_, err := jwk.RSAKey()
		assert.Error(t, err)
...
func TestJWKS_GetKeyByKID(t *testing.T) {
	...
	t.Run("returns RSA public key", func(t *testing.T) {
		...
		assert.Equal(t, 0, key1.N.Cmp(pubKey.N))
		assert.Equal(t, key1.E, pubKey.E)
	})
