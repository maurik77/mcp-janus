...
func TestNew(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		jwksServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
			json.NewEncoder(w).Encode(JWKS{Keys: []JWK{rsaPublicKeyToJWK(&key.PublicKey, "kid-1")}})
		}))
...
func TestValidateJWT(t *testing.T) {
	...
	refreshOK := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(JWKS{Keys: []JWK{rsaPublicKeyToJWK(&key.PublicKey, kid)}})
	}))
	...
	refreshMiss := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(JWKS{Keys: []JWK{rsaPublicKeyToJWK(&key.PublicKey, "different-kid")}})
	}))
